...
library(ropls)
x=read.csv("Supplementary Fig.3a mND_vs_mHFD_OPLSDA.csv")
row.names(x)=x[,1]
x=x[,-1]
x = t(x)
group=c(rep('mND',9),
...
library(tidyr)
library(dplyr)
data=read.csv("Fig.2a mND_vs_mHFD_PCA.csv")
...
library(ggpubr)
library(ggthemes)   
data=read.csv("Supplementary Fig.3b mND_vs_mHFD_VolcanoPlot.csv")
head(data)
...
library("pheatmap")
x=read.csv("Fig.2c mND_vs_mHFD_diff_heatmap_Vitamin.csv")  ### or x=read.csv("mND_vs_mHFD_diff_heatmap_Vitamin.csv") 
row.names(x)=x[,1]
x=x[,-1]
group=c(rep('mND',9),
            rep('mHFD',9))
...
rm(list = ls())
data<-read.csv('Supplementary Fig.3c Metabolites_phenotypes_Correlation.csv', row.names = 1)
data <- t(data)
library(corrplot)
library(psych)
...
library(ggpubr)
library(psych)
data <- read.delim('Fig.2c mND_vs_mHFD_diff_heatmap_Vitamin.csv', row.names = 1, sep = ",", stringsAsFactors = FALSE, check.names = FALSE)
data <- t(data)
...
#####16S-seq Original code
###PCOA
library(vegan)  
library(ggplot2)
library(plyr)
otu1=read.csv("Supplementary Fig.4e OTU_PCOA.csv") 
...
###Barplot
rm(list = ls())
phylum <-  read.delim('Supplementary Fig.4f Phylum_top10.csv', row.names = 1, sep = ",", stringsAsFactors = FALSE, check.names = FALSE)
...
rm(list = ls())
library(pheatmap)
x=read.csv("Supplementary Fig.4h Heatmap.csv")
row.names(x)=x[,1]
x=x[,-1]
x1=log2(x + 1)
group=c(rep('mND',10),
